...
| 7 | Progress Bar Pra-proses Otomatis | Elemen Indikator Progres | Batang indikator penanda selesainya tahap *Resizing* ke $224 \times 224$ dan normalisasi piksel $[0, 1]$. |
...
| 10 | Tombol Mulai Analisis CNN | Elemen Interaktif (Primary Button) | Tombol aksi utama di kanan bawah untuk memulai kalkulasi dan lanjut ke tahap konvolusi (`pages/02_conv.html`). |
...
| 2 | Card Demo Cara Kerja Conv1 | Elemen Visualisasi Interaktif | Tampilan rumus perkalian elemen $y = \sum (X_{i,j} \times W_{i,j})$ dan petunjuk tanpa bias (`use_bias=False`). |
| 3 | Visualisator Patch $3 \times 3$ & Bobot W | Elemen Visualisasi Data | Grid petak mini perbandingan patch piksel citra ($X$) dikalikan bobot filter real-time ($W$) menghasilkan nilai output mentah ($y$). |
...
| 5 | Tabel Perhitungan Empiris Dot Product | Elemen Tabel Data | Tabel rincian kalkulasi 9 posisi spasial $(i,j)$ perkalian sel $X \times W$ hingga perkalian kumulatif total dot product $+0.05028$. |
...
| 6 | Tabel Spesifikasi Parameter ReLU | Elemen Tabel Parameter | Tabel ringkas metrik ReLU ($0$ parameter terlatih, ukuran tensor tetap $224 \times 224 \times 32$). |
...
| 2 | Visualisator Max Pooling $2 \times 2$ | Elemen Visualisasi Data | Grid perbandingan sebelum (4×4 aktivasi) dan sesudah (2×2 output) yang mengambil nilai tertinggi tiap jendela $2 \times 2$. |
| 3 | Tabel Perhitungan Empiris Max Pooling | Elemen Tabel Data | Tabel rincian 4 jendela $2 \times 2$ non-overlapping beserta nilai masukan dan nilai maksimum terambil. |
...
| 5 | Tabel Parameter 3 Tahap Pooling | Elemen Tabel Parameter | Spesifikasi ukuran tensor dari Pool1 ($112 \times 112 \times 32$), Pool2 ($56 \times 56 \times 64$), hingga Pool3 ($28 \times 28 \times 128$). |
...
| No. | Nama Komponen | Jenis Elemen | Deskripsi |
| :---: | :--- | :--- | :--- |
| 1 | Header Langkah 5 | Elemen Header Teks | Penanda "Tahap 5 dari 7" dan penjelasan perataan tensor 3D ($7 \times 7 \times 1280$) menjadi vektor 1D ($1.280$ elemen). |
| 2 | Display Rumus & Panduan Cara Pakai GAP | Elemen Visualisasi Formula | Rumus $y = \frac{1}{49} \sum X_{7 \times 7}$ dan rincian 3 langkah kalkulasi (Kumpulkan 49 piksel $\to$ Penjumlahan $\to$ Pembagian N=49). |
| 3 | Diagram 3D Tensor Masuk | Elemen Visualisasi 3D | Visualisasi tumpukan 1.280 feature maps ukuran $7 \times 7$ piksel. |
...
| 8 | Skema Diagram Transformasi Dimensi | Elemen Diagram Visual | Diagram alur perubahan dimensi $7 \times 7 \times 1280 \to \text{GAP} \to 1280 \times 1$. |
...
| 4 | Display Rumus Logit & Softmax | Elemen Visualisasi Formula | Rumus gabungan perkalian bobot $z_k = \sum (X_i \times W_{i,k}) + b_k$ dan normalisasi eksponensial $P(y_k) = \frac{e^{z_k}}{\sum e^{z_j}}$. |
...
| 8 | Tabel Ringkasan Proses CNN | Elemen Tabel Parameter | Tabel komparatif alur komputasi dari input $224 \times 224 \times 3$, 3 blok konvolusi, pooling, GAP, FC, hingga Softmax output. |
