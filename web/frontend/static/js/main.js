...
async function goTo(n) {
  ...
  try {
    const response = await fetch(pageFiles[n], { cache: 'no-store' });
    if (!response.ok) throw new Error(`Gagal memuat ${pageFiles[n]}`);
...
async function uploadFile(file) {
  ...
  try {
    ...
    toast.remove();
    // Pesan ini yang muncul di tangkapan layar Anda
